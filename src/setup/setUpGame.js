...
function onOpen() {
  ...
  ui.createMenu("Play FR")
    ...
    .addItem("Restore Game", "resetGameTurn")
 ...
 * @param {number} d1ex - residual exhaustion from player data
 * @param {number} d2ex - residual exhaustion from player data
 * @returns {Array of objects} decks - array of two objects representing the starting decks
 */
...
/**
 * initial function to setup game based on data from baseGameInfo sheet
 * once game and new sheets created, emailing players begins
 */
function setUpGame() {
...
/**
 * returns game to previous state saved by createRestorePoint function
 */
function resetGameTurn() {
  // TODO adjust this function to take into account the new sheets added
  let trackRestore = ss
    .getSheetByName("trackRestore")
    .getDataRange()
    .getValues();
  ...
  track.clearContents();
  track
    .getRange(1, 1, trackRestore.length, trackRestore[0].length)
    .setValues(trackRestore);
...
function createRestorePoint() {
  ...
  trackRestore
    .getRange(1, 1, trackInfo.length, trackInfo[0].length)
    .setValues(trackInfo);

  dbRestore.clearContents();
  dbRestore
    .getRange(1, 1, dbValues.length, dbValues[0].length)
    .setValues(dbValues);
}
